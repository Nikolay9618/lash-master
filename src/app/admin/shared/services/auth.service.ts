...
export class AuthService {
   ...
   get token(): any {
      const expDate = new Date(localStorage.getItem('fb-token-exp') ?? new Date())
      if (new Date() > expDate) {
         this.logout()
         return null
      }
   ...
   private handleError(error: HttpErrorResponse) {
      ...
      switch (message) {
         ...
         case 'EMAIL_NOT_FOUND':
            this.error$.next('Пользователь с таким email не существует')
            break
      }
   ...
   private setToken(response: FbAuthResponce | any) {
      if (response) {
